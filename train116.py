...
def loss_with_ignore(out, truth, ignore_mask):
    # Rescale output to be from -1 to 1
    tmp_out = out[:,1,:,:]
    tmp_out = (tmp_out * 2) - 1
    tmp_truth = 1 - (truth * 2)
    loss = tmp_truth.type(torch.cuda.FloatTensor) * tmp_out
    # Lets do the hinge loss thing (0.1 margin on both sides)
    loss = loss + 0.1
    loss = torch.clamp(loss, 0, 1.0)
    # set the pixels to ignore to have loss 0
    loss[ignore_mask > 0] = 0
    # average it down into a single number for each image
    loss = torch.sum(loss, 2)
    loss = torch.sum(loss, 1)
    # average of pixels not ignored
    pixel_count = (1 - ignore_mask)
    pixel_count = torch.sum(pixel_count,2)
    pixel_count = torch.sum(pixel_count,1)
    pixel_count[pixel_count == 0] = 1
    image_loss = loss / pixel_count.type(torch.cuda.FloatTensor)
    return image_loss
...
def train(net, data, params):
    ...
    for batch in range(params['num_batches']):
        ...
        for mini in range(params['num_minibatches']):  # loop over the dataset multiple times
            ...
            # Compute my own loss
            tmp_out = smax(tmp_out)
            my_loss = loss_with_ignore(tmp_out, truth_tensor, ignore_mask_tensor)
            
            # print statistics
            running_loss = my_loss.mean().item() * 1000
            if mini == 0:
                # Generate error for each truth pixel.
                # TODO: Make this go from 0 to 1 (soft max)
                loss_np = my_loss.detach().cpu().numpy()
                num = loss_np.shape[0]-num_adversarial_images
